...
<div id="#hotel-booking-cart" class="cartPageWrap checkout">
	...
	<div id="hotel-booking-payment">
		...
		<form name="hb-payment-form" id="hb-payment-form" method="post"
			...
			<h3 class="hb-title"><?php _e( 'Booking Domes', 'wp-hotel-booking' ); ?></h3>
				...
				<?php if ( $rooms = $cart->get_rooms() ) {
					foreach ( $rooms as $cart_id => $room ) {
						...
						<tr class="hb_checkout_item" data-cart-id="<?php echo esc_attr( $cart_id ); ?>">
							<td class="hb_room_type"<?php echo defined( 'WPHB_EXTRA_FILE' ) && $cart_extra ? ' rowspan=""' : '' ?>>
			...
			<?php } else { ?>
				...
				<?php } ?>
				<div class="btnArea">
					<button type="submit" class="hb_button btn btn-secondary"><?php _e( 'Check out', 'wp-hotel-booking' ); ?></button>
				</div>
