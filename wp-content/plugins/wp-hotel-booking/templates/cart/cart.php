...
<div class="cartPageWrap">
	<div class="fusion-row">
		<?php if ( $cart->cart_items_count != 0 ) { ?>
			<div id="hotel-booking-cart" class="cartPage">

				<form id="hb-cart-form" method="post">

					<h3 class="hb-title"><?php _e( 'Cart', 'wp-hotel-booking' ); ?></h3>

					<table class="hb_table cartPageTable">
						<thead>
						<tr>
							<th>&nbsp;</th>
							<th class="hb_room_type"><?php _e( 'Room type', 'wp-hotel-booking' ); ?></th>
							<th class="hb_capacity"><?php _e( 'Capacity', 'wp-hotel-booking' ); ?></th>
							<th class="hb_quantity"><?php _e( 'Dome', 'wp-hotel-booking' ); ?></th>
						...
						<?php if ( $rooms = $cart->get_rooms() ) { ?>
							<?php foreach ( $rooms as $cart_id => $room ) { ?>
								...
								<tr class="hb_checkout_item" data-cart-id="<?php echo esc_attr( $cart_id ); ?>">
									<td<?php echo defined( 'WPHB_EXTRA_FILE' ) && $cart_extra ? ' rowspan=""' : '' ?>>
					...
					<div class="btnArea">
						<a href="<?php echo hb_get_checkout_url() ?>" class="hb_checkout btn btn-secondary"><?php _e( 'Check Out', 'wp-hotel-booking' ); ?></a>
					</div>
				</form>
			</div>
