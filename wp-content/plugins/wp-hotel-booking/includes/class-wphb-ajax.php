...
class WPHB_Ajax {
	...
	public static function add_extra_to_cart() {
		...
		hb_send_json( array( 'status' => 'success', 'redirect' => $pageRedirect ) );
	}
	...
	static function ajax_add_to_cart() {
		...
		if ( ! isset( $_POST['room-id'] ) || ! isset( $_POST['hb-num-of-rooms'] ) ) {
			$result['message'] = __( 'Dome ID is not exists.', 'wp-hotel-booking' );
			hb_send_json( $result );
		}
		...
		if ( ! is_wp_error( $cart_item_id ) ) {
			...
			hb_send_json( array(
				'status'  => 'warning',
				'message' => __( 'Dome selected. Please View Cart to change order', 'wp-hotel-booking' )
			) );
		}
	}
	...
	static function check_room_available() {
		...
		if ( ! isset( $_POST['product_id'] ) || ! $_POST['product_id'] ) {
			wp_send_json( array(
				'status'  => false,
				'message' => __( 'Dome not found.', 'wp-hotel-booking' )
			) );
		}
	...
	static function load_other_full_calendar() {
		...
		if ( ! isset( $_POST['room_id'] ) ) {
			wp_send_json( array(
				'status'  => fasle,
				'message' => __( 'Dome is not exists.', 'wp-hotel-booking' )
			) );
		}
